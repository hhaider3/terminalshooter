...
The Rust game is now the default; see the root README. The experimental native
mouse lock has been removed from the Python version.
...
Mouse aiming uses ordinary terminal events. The pointer remains free and stops
at window edges; Q/E can always turn the camera. L toggles aiming and resets the
movement baseline. The HUD shows `Mouse ON` or `OFF`. There is no native cursor
locking, recentering, or macOS pointer helper. Menus have their own background,
with no crosshair or weapon drawn underneath.
...
The F toggle avoids relying on repeat for firing. Mouse motion stops at window
edges; Q/E can always turn you around. Terminal modes are restored when you exit.
